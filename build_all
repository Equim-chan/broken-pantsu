...
if ! hash sha1sum 2>/dev/null; then
    if ! hash sha256sum 2>/dev/null; then
        echo "Neither 'sha1sum' nor 'sha256sum' was found."
...
for os in ${OSES[@]}; do
    for arch in ${ARCHS[@]}; do
        ...
        env CGO_ENABLED=0 \
            ...
            -o ${SOFTWARE}_${os}_${arch}${suffix} $SOURCE
...
for v in ${ARMS[@]}; do
    env CGO_ENABLED=0 \
        ...
        go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" \
        -o ${SOFTWARE}_linux_arm$v $SOURCE
done
...
env CGO_ENABLED=0 \
    ...
    go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" \
    -o ${SOFTWARE}_linux_mipsle $SOURCE
env CGO_ENABLED=0 \
    GOOS=linux GOARCH=mips \
    go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" \
    -o ${SOFTWARE}_linux_mips $SOURCE
